...
public class UploadVideoService 
    ...
    public static final String ACTION_UPLOAD_SERVICE_RESPONSE =
                "com.example.services.UploadVideoService.RESPONSE";
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        try {
        	status = mVideoMediator.uploadVideo(getApplicationContext(),
                    intent.getData());
			finishNotification(status);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			status = VideoDataMediator.STATUS_UPLOAD_ERROR;
		}
    ...
    private void sendBroadcast(String status){
    	...
    	Intent statusIntent = new Intent(ACTION_UPLOAD_SERVICE_RESPONSE).addCategory(Intent.CATEGORY_DEFAULT);
    	statusIntent.putExtra("VSTATUS", status);
